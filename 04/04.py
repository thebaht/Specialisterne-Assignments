...
def eval1(reader):
    source.seek(0)  # start from the beginning of the file
    ...
    return lists    # return list<list> for charts
...
file_path = 'less_evals.csv' # file to read from       
# file_path = 'evals.csv'   # file to read from
try:     
    with open(file_path, 'r') as source: # open file
        reader = csv.reader(source) # create reader
        e1 = eval1(reader)  # run the 1st evaluation 
        e2 = eval2(reader)  # run the 2nd evaluation
